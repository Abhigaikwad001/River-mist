...
export default function RiversidePage() {
  ...
  const fetchActivities = async () => {
    try {
      const res = await api.get('/activities?activeOnly=true&category=RIVERSIDE');
      setActivities(res.data);
    } catch (err) {
      console.error('Failed to load riverside activities:', err);
      setActivities([
        { id: 1, name: 'Riverside Sunset Walk', description: 'Serene walking trail along the river bank.', price: 0 },
        { id: 2, name: 'Evening Bonfire & Music', description: 'Cozy evening gathering around the campfire with soft acoustic music.', price: 0 }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="bg-white min-h-screen">
      <div className="relative h-[55vh] bg-[#1E3F20] flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4">
          <Compass className="w-14 h-14 text-[#D4AF37] mx-auto mb-3" />
          <h1 className="text-4xl md:text-6xl font-serif font-bold text-white mb-3 drop-shadow-md">Riverside Trails</h1>
          <p className="text-lg text-gray-200 max-w-2xl mx-auto font-light">Peaceful riverfront walks, sunset viewpoints, and evening campfire gatherings.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16 max-w-5xl">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 space-y-3">
            ...
            <p className="text-[#1E3F20] font-serif text-sm">Loading riverside activities...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            {activities.length > 0 ? activities.map((act) => (
              <div key={act.id} className="bg-[#FAF9F6] p-8 rounded-3xl text-center shadow-sm border border-[#D4AF37]/20 hover:shadow-md transition-shadow">
                <Compass className="w-10 h-10 text-[#D4AF37] mx-auto mb-4" />
              ...
              <div className="col-span-2 text-center py-8 text-gray-500 font-serif">
                No specific riverside activities currently listed.
              </div>
            )}
          </div>
        )}
